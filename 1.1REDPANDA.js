...
function saveAndExit(command) {
  ...
  saveToFile(filename, editor.lines);
  console.log('Exiting TermaCode. Goodbye!');
  process.exit(0);
}
...
function saveToFile(filename, content) {
  ...
  fs.writeFileSync(saveFilename, content.join(os.EOL), 'utf-8');
}
...
function openFile(command) {
  ...
  try {
    ...
    console.error(`Error opening file "${customFilename}":`, error);
  }
...
// Function to construct header strings
function constructHeaderStrings() {
  const title = 'TermaCode V1.1  ';
  const line = '|'.padStart(HEADER_WIDTH + 2, '-').padEnd(HEADER_WIDTH + 2, '-');
  const padding = `|${' '.repeat(HEADER_WIDTH)}|`;

  return [
    line,
    padding,
    `|${title.padStart(HEADER_WIDTH).padEnd(HEADER_WIDTH)}|`,
    padding,
    line,
    '',
  ];
...
function printHeader() {
  const [line, padding, titleLine, paddingAfterTitle, bottomLine, emptyLine] = constructHeaderStrings();

  console.log(line);
  console.log(padding);
  console.log(titleLine);
  console.log(paddingAfterTitle);
  console.log(bottomLine);
  console.log(emptyLine);
}


// Function to confirm and exit
